...
airborne.bootstrap.obj.__action = {
	...
	,is_valid_action_item_obj:function(action_item_obj){
		...
		if(action_item_obj == undefined) {
			...
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_item_obj / action_item_obj.get_action_item_type == undefined");
			return false;
		} else if(_v.is_not_unsigned_number(action_item_obj.get_action_item_type())) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_item_obj / _v.is_not_unsigned_number(action_item_obj.get_action_item_type())");
			return false;
		} else if(action_item_obj.get_action_item_type_name == undefined) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_item_obj / action_item_obj.get_action_item_type_name == undefined");
			return false;
		} else if(_v.is_not_valid_str(action_item_obj.get_action_item_type_name())) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_item_obj / _v.is_not_valid_str(action_item_obj.get_action_item_type_name())");
			return false;
		}
	...
	,is_valid_action_obj:function(action_obj){
		...
		if(action_obj == undefined) {
			...
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_obj / action_obj.get_action_id == undefined");
			return false;
		} else if(action_obj.get_action_name == undefined) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_obj / action_obj.get_action_name == undefined");
			return false;
		} else if(_v.is_not_valid_str(action_obj.get_action_name())) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_obj / _v.is_not_valid_str(action_obj.get_action_name())");
			return false;
		} else if(action_obj.get_action_context == undefined) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_obj / action_obj.get_action_context == undefined");
			return false;
		} else if(action_obj.get_action_hash_key == undefined) {
			// 반드시 있어야할 속성이므로 에러 메시지를 노출합니다.
			console.log("!Error! / is_valid_action_obj / action_obj.get_action_hash_key == undefined");
			return false;
		}

		if(this.has_infinite_loop(action_obj)) {
			return false;
		}

		return true;
	}	
	,has_infinite_loop:function(target_action_obj) {

		if(target_action_obj == undefined) {
			return false;
		}

		// check backward
		var max_loop_cnt = 100;
		var is_over_max_loop_cnt = true;
		var action_obj_after = target_action_obj;
		for(var idx = 0; idx < max_loop_cnt; idx++) {
			action_obj_after = action_obj_after.get_sibling_action_obj_after();
			if(action_obj_after == undefined) {
				is_over_max_loop_cnt = false;
				break;
			}
		}

		if(is_over_max_loop_cnt) {
			console.log("!Error! / has_infinite_loop / is_over_max_loop_cnt");
			return true;
		}

		// check forward
		is_over_max_loop_cnt = true;
		var action_obj_before = target_action_obj;
		for(var idx = 0; idx < max_loop_cnt; idx++) {
			action_obj_before = action_obj_before.get_sibling_action_obj_before();
			if(action_obj_before == undefined) {
				is_over_max_loop_cnt = false;
				break;
			}
		}
	...
	,get_action_obj_empty:function() {
		...
		var action_obj = {
			...
			,copy:function(new_action_name, parent_action_obj, src_action_obj, is_shy) {
				...
				if(_action.is_not_valid_action_obj(action_obj_copy)) {
					console.log("!Error! / copy / _action.is_not_valid_action_obj(action_obj_copy)");
					return;
				}
				...
				consoler.say("copy / 000 / action copy : ",action_obj_copy.get_coordinate());
				consoler.say("copy / 000 / action copy.has_before() : ",action_obj_copy.has_before());
				consoler.say("copy / 000 / action copy.has_after() : ",action_obj_copy.has_after());
				...
				// 자식 shy 객체를 하나 추가합니다. 새로 복사한 
				// 조건 
				// 1.복사 대상에 자식이 있어야 합니다.
				// 2.있다면 복사 대상의 첫번재 객체만 복사합니다.
				...
				if(cur_sibling_action_obj_after != undefined && _action.is_not_valid_action_obj(cur_sibling_action_obj_after)) {
					console.log("!Error! / copy / _action.is_not_valid_action_obj(cur_sibling_action_obj_after)");
					return;
				}

				// 이제부터 src_action_obj의 형제 관계가 변경됩니다. src_action_obj의 데이터를 참조하면 무한 루프가 발생할 수 있습니다.
				src_action_obj.set_sibling_action_obj_after(action_obj_copy);
				action_obj_copy.set_sibling_action_obj_before(src_action_obj);

				if(src_action_obj.has_no_after()) {
					console.log("!Error! / copy / src_action_obj.has_no_after()");
					return;
				} else if(action_obj_copy.has_no_before()) {
					console.log("!Error! / copy / action_obj_copy.has_no_before()");
					return;
				}
				...
				consoler.say("copy / 001-2 / after action : ",action_obj_copy);
				consoler.say("copy / 001-2 / after action : ",action_obj_copy.get_coordinate());

				// 다음 객체가 있었다면 앞 뒤 형제 관계를 만들어줍니다.
				var cur_sibling_action_obj_after_after = undefined;
				if(cur_sibling_action_obj_after != undefined) {

					consoler.say("copy / 002-1 / after action : ",action_obj_copy);
					consoler.say("copy / 002-1 / after action : ",action_obj_copy.get_action_name());
					consoler.say("copy / 002-2 / after-after action : ",cur_sibling_action_obj_after);
			...
			,dive_into_indixes:function(idx_before_action_item_obj, idx_after_action_item_obj) {
				...
				if( new_before_action_item_obj != undefined && 
					new_before_action_item_obj.get_coordinate() === this.get_coordinate()) {

					// 앞의 형제 아이템이 자신과 동일한 좌표값을 가지면 중단합니다.
					console.log("!Error! / dive_into_indixes / new_before_action_item_obj.get_coordinate() === this.get_coordinate()");
					return;
				...
				if( new_after_action_item_obj != undefined && 
					new_after_action_item_obj.get_coordinate() === this.get_coordinate()) {

					// 뒤의 형제 아이템이 자신과 동일한 좌표값을 가지면 중단합니다.
					console.log("!Error! / dive_into_indixes / new_after_action_item_obj.get_coordinate() === this.get_coordinate()");
					return;
			...
			,set_sibling_action_obj_before:function(sibling_action_obj_before) {
				...
				if(sibling_action_obj_before != undefined) {
					...
					if(has_infinite_loop) {
						console.log("!Error! / set_sibling_action_obj_after / has_infinite_loop / ",sibling_action_obj_before);
						return;
					}
				}

				if(sibling_action_obj_before != undefined) {
					if(this.get_coordinate() === sibling_action_obj_before.get_coordinate()) {
						console.log("!Error! / set_sibling_action_obj_after / this.get_coordinate() === sibling_action_obj_before.get_coordinate()");
						return;
					}
				}
			...
			,has_no_before:function() {
				return !this.has_before();
			}
			,has_before:function() {
				var cur_sibling_action_obj_before = this.get_sibling_action_obj_before();
				if(_action.is_valid_action_obj(cur_sibling_action_obj_before)) {
					return true;
				}
				return false;
			}
			...
			,set_sibling_action_obj_after:function(sibling_action_obj_after) {
				...
				if(sibling_action_obj_after != undefined) {
					// CHECK 무한 루프가 만들어지는지 검사합니다. - 형제 객체 연결을 끊는 방법으로 undefined 지정할 수 있는 경우를 고려 
					var has_infinite_loop = _action.has_infinite_loop(sibling_action_obj_after);
					if(has_infinite_loop) {
						console.log("!Error! / set_sibling_action_obj_after / has_infinite_loop / ",sibling_action_obj_after);
						return;
					}
				}

				if(sibling_action_obj_after != undefined) {
					if(this.get_coordinate() === sibling_action_obj_after.get_coordinate()) {
						console.log("!Error! / set_sibling_action_obj_after / this.get_coordinate() === sibling_action_obj_after.get_coordinate()");
						return;
					}
				}
			...
			,has_no_after:function() {
				return !this.has_after();
			}
			,has_after:function() {
				var cur_sibling_action_obj_after = this.get_sibling_action_obj_after();
				if(_action.is_valid_action_obj(cur_sibling_action_obj_after)) {
					return true;
				}
				return false;
			}
			...
			,get_action_context_obj:function() {
			...
			// @ Public
			// @ Scope : action obj
			// @ Desc  : action_id, action_hash_key, action_item_type, action_name, action_context만을 가지고 있는 객체를 돌려줍니다. 어떤 객체라도 1개의 부모만 가질수 있습니다. DB 업데이트 전용으로 사용합니다.
			,get_action_obj_for_db_update:function() {
		...
		// 기본 검사 항목은 기본값으로 채워줍니다.
		action_obj.set_action_name(_param.PLACE_HOLDER_NEW_ITEM);
		action_obj.set_action_context("");
		action_obj.set_action_hash_key("");
		...
		var element_event_manager = {
			...
			,call_delegate_save_n_reload:function(cur_element_type, cur_event_type, cur_search_option_obj){
				...
				if( (cur_action_item_obj.is_item_select_box() || cur_action_item_obj.is_item_select_box_addable()) && _action.EVENT_TYPE_UPDATE_ITEM == cur_event_type ){
					...
					var cur_action_context_obj = cur_action_item_obj.get_action_context_obj();
					if(cur_action_context_obj == undefined) {
						cur_action_context_obj = {};
					}

					cur_action_context_obj[_param.SELECTED_KEY] = cur_element_key;
					cur_action_context_obj[_param.SELECTED_VALUE] = cur_element_value;

					cur_action_item_obj.set_action_context(JSON.stringify(cur_action_context_obj));
			...
			,set_before_n_after_siblings_event_manager:function(new_before_sibling_event_manager, new_after_sibling_event_manager){
				...
				if( action_item_obj_before != undefined && 
					action_item_obj_before.get_coordinate() === cur_action_item_obj.get_coordinate()) {

					// 앞의 형제 아이템이 자신과 동일한 좌표값을 가지면 중단합니다.
					console.log("!Error! / set_before_n_after_siblings_event_manager / action_item_obj_before.get_coordinate() === cur_action_item_obj.get_coordinate()");
					return;
				...
				if( action_item_obj_after != undefined && 
					action_item_obj_after.get_coordinate() === cur_action_item_obj.get_coordinate()) {

					// 앞의 형제 아이템이 자신과 동일한 좌표값을 가지면 중단합니다.
					console.log("!Error! / set_before_n_after_siblings_event_manager / action_item_obj_after.get_coordinate() === cur_action_item_obj.get_coordinate()");
					return;

				}

				if( action_item_obj_before != undefined && 
					action_item_obj_after != undefined && 
					action_item_obj_before.get_coordinate() === action_item_obj_after.get_coordinate() ) {

					console.log("!Error! / set_before_n_after_siblings_event_manager / action_item_obj_before.get_coordinate() === action_item_obj_after.get_coordinate()");
					return;
				}


				// Event Manager의 관계 변경
				// A->B->C ===> A->C
		...
		var element_collection_set = {
			...
			*/
			, set_jump_event:function(cur_btn_collection_eject_jq) {
				...
				// 기본 정보 
				// 사용자 선택한 엘리먼트의 처음 너비. 이 너비로 이동합니다.
				...
				if(cur_element_collection_container_jq == undefined) {
					console.log("!Error! / set_jump_event / cur_element_collection_container_jq == undefined");
					return;
				}
				...
				cur_btn_collection_eject_jq.click(function(event_click){
					...
					clone_element_collection_container_jq.click(function(e){
						...
						if(_action.is_valid_action_obj(cur_parent_action_obj_on_mouse_over)) {
							...
							var table_element_collection_container_jq = cur_table_action_obj.get_table_element_collection_set().get_element_collection_container_jq();
							if(table_element_collection_container_jq == undefined) {
								console.log("!Error! / clone_element_collection_container_jq.click / table_element_collection_container_jq == undefined")
								return;
							}
							cur_parent_action_obj_on_mouse_over.get_event_manager().push_add_on_element_jq(table_element_collection_container_jq);
...
Event Manager
	...
	- set_before_n_after_siblings_event_manager 
		객체 이동시 event manager간의 관계를 변경해줍니다. element set, action item의 관계도 바꾸어 줍니다.
